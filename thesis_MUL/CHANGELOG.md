...
Each version below is frozen and available as a git tag (`thesis-v<version>`); only the current version's files are kept in the working tree (flat, directly in `thesis_MUL/`). Archived versions' own `README.md` can be read by checking out their tag, e.g. `git show thesis-v1.1:thesis_MUL/README.md`.
